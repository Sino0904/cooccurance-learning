...
def init_pretrained_weights_symm(model, model_symm):
    """
    Initialize model with pretrained weights.
    Layers that don't match with pretrained layers in name or size are kept unchanged.
    """
    ...
    model_dict.update(pretrain_dict)
    model.load_state_dict(model_dict)
    model.classifier = torch.nn.Linear(2048, 79)
...
def resnet50(num_classes, pretrained='imagenet',model_load=None,use_selfatt=False, **kwargs):
    ...
    elif pretrained == 'symmetric':
        init_pretrained_weights_symm(model, model_load)
    return model
