...
class Engine(object):
    ...
    def learning(self, model, criterion, criterion_val, train_dataset, val_dataset, optimizer=None):
        ...
        if self.state['evaluate']:
            self.validate(val_loader, model, criterion_val)
            return
        ...
        for epoch in range(self.state['start_epoch'], self.state['max_epochs']):
            ...
            prec1 = self.validate(val_loader, model, criterion_val)
    ...
    def test(self, model, criterion, test_dataset, optimizer=None):
        ...
        with torch.no_grad():
            ...
            self.on_start_epoch(False, model, criterion, test_loader)
            ...
            for i, (input, target) in enumerate(test_loader):
                ...
                self.on_start_batch(False, model, criterion, test_loader)

                if self.state['use_gpu']:
                    self.state['target'] = self.state['target'].cuda(async=True)

                self.on_forward(False, model, criterion, test_loader)
                ...
                # measure accuracy
                self.on_end_batch(False, model, criterion, test_loader)

            score = self.on_end_epoch(False, model, criterion, test_loader)
...
class GCNMultiLabelHNHEngine(HNHMultiClassEngine):
    def on_forward(self, training, model, criterion, data_loader, optimizer=None, display=True):
        feature_var = torch.autograd.Variable(self.state['feature']).float()
        target_var = torch.autograd.Variable(self.state['target']).float()
