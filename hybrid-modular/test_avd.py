...
parser.add_argument('--model_type', type=str, default='l1')
...
#     model = models.init_model(name=args.arch, num_classes=len(CLASSES), pretrained = None, use_gpu=True)
    ...
    return feature_model, antisymm_classifier_model
    

def test_per_image(image_name,feature_model, antisymm_classifier_model):
...
def test_per_set(img_paths, target_label,feature_model, antisymm_classifier_model):
    #img_paths = [os.path.join(args.dataset_path,"test", x) for x in img_set]
    imgs_scores = []
    for i in range(len(img_paths)):
        pred_class, class_prob, _ = test_per_image(img_paths[i], feature_model, antisymm_classifier_model)
...
def main():
    ...
    #result = {}
    cor = 0
    feature_model, antisymm_classifier_model = get_model()
    for i in range(len(gallery_list)):
        ...
        scores_all = test_per_set(img_paths, target_label, feature_model, antisymm_classifier_model)
