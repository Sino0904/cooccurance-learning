...
def init_pretrained_weights_antisymm(model, model_antisymm):
    """
    Initialize model with pretrained weights.
    Layers that don't match with pretrained layers in name or size are kept unchanged.
    """
...
def resnet50(num_classes, pretrained='imagenet',use_selfatt=False,model_load=None, **kwargs):
    model = ResNet(
    ...
    if pretrained == 'antisymmetric':
        init_pretrained_weights_antisymm(model, model_load)
    return model
